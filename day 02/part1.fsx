...
type SubmarineState = { HorizontalPosition: int; Depth: int }
let initialState = { HorizontalPosition = 0; Depth = 0 }
...
let applyCommand state command =
    ...
    | Down d -> { state with Depth = state.Depth + d }
    | Up d -> { state with Depth = state.Depth - d }
...
let run () =
    ...
    test <@ solve example = { HorizontalPosition = 15; Depth = 10 } @>
    printfn "...done!"
